...
titles_and_funcs = [
    ("Prix (Put)", bsm_put_price),
    (r"Delta ($\Delta$)", bsm_put_delta),
    (r"Gamma ($\Gamma$)", bsm_gamma),
    (r"Theta 1D ($\Theta$)", bsm_put_theta),
    (r"Vega ($\nu$)", bsm_vega),
    (r"Rho ($\rho$)", bsm_put_rho)
]
...
for idx, (title, func) in enumerate(titles_and_funcs):
    ...
    if idx == 0: # Add legend only to the first subplot to save space
        ax.legend(title="Maturité", fontsize=9, loc='upper right')

# Super Title
fig.suptitle(r"Matrice 2D des Sensibilités Black-Scholes (Option PUT)" + f"\nStrike K={K}, Taux r={r*100}%, Volatilité $\\sigma$={sigma*100}%",
             fontsize=16, fontweight='bold', y=0.98)

plt.tight_layout(rect=[0, 0, 1, 0.94])
print("Graphique Matrice 2D des Grecques (PUT) terminé. Affichage...")
plt.show()
