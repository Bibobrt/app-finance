...
# Calculate all surfaces for PUT
Price = bsm_put_price(S, K, T_grid, r, sigma)
Delta = bsm_put_delta(S, K, T_grid, r, sigma)
Gamma = bsm_gamma(S, K, T_grid, r, sigma)
Theta = bsm_put_theta(S, K, T_grid, r, sigma)
Vega  = bsm_vega(S, K, T_grid, r, sigma)
Rho   = bsm_put_rho(S, K, T_grid, r, sigma)
...
for idx, (Z_data, title, cmap) in enumerate(metrics, start=1):
    ...
    if "Gamma" in title or "Vega" in title:
        ...
        ax.view_init(elev=25, azim=50)   # Different for Put Delta (negative)
    elif "Rho" in title:
        ax.view_init(elev=25, azim=50)   # Rho for Put is negative
    else:
        ax.view_init(elev=25, azim=230)  # Classic view
        
    ax.invert_xaxis() # Pour que T=0 soit au fond ou devant selon l'angle, plus logique temporellement

# Super Title (used raw string r"..." to fix the LaTeX Warning on \sigma)
fig.suptitle(r"Matrice 3D des Sensibilités Black-Scholes (Option PUT)" + f"\nStrike K={K}, Taux r={r*100}%, Volatilité $\\sigma$={sigma*100}%",
             fontsize=18, fontweight='bold', y=0.98)

plt.tight_layout(rect=[0, 0, 1, 0.95]) # Leave space for suptitle
plt.subplots_adjust(wspace=0.1, hspace=0.1)

print("Graphique Matrice 3D des Grecques (PUT) terminé. Affichage...")
plt.show()
