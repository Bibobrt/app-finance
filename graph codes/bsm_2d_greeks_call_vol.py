...
for idx, (title, func) in enumerate(titles_and_funcs):
    ...
    if idx == 0: # Add legend only to the first subplot to save space
        ax.legend(title="Volatilité", fontsize=9, loc='upper left')

# Super Title
fig.suptitle(r"Matrice 2D des Sensibilités Black-Scholes (LONG CALL)" + f"\nStrike K={K}, Taux r={r*100}%, Maturité T={int(T_fixed*365)} jours",
             fontsize=16, fontweight='bold', y=0.98, color='darkgreen')

plt.tight_layout(rect=[0, 0, 1, 0.94])
print("Graphique Matrice 2D des Grecques (CALL vs VOLATILITÉ) terminé. Affichage...")
plt.show()
