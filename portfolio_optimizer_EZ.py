...
class MeanVarianceOptimizer:
    ...
    def plot_frontier(self, risk_free_rate=None):
        """
        Plots the Efficient Frontier, optimized portfolios, and Capital Market Line (CML).
        """
        ...
        for ticker in self.tickers:
            ...
            try:
                ...
                plt.scatter(asset_vol, asset_ret, s=50, label=ticker, alpha=0.6)
                plt.annotate(ticker, (asset_vol, asset_ret), xytext=(5, 5), textcoords='offset points', fontsize=8)
            except:
                pass

        plt.title(f'Efficient Frontier & CML ({len(self.tickers)} Assets)\nRisk-Free Rate: {risk_free_rate:.2%}')
